function [denoised_image] = HuberFunction(noisy_image, var, alpha, gamma)
...
while diff>0.01 && step>0.0001
    ...
    cost_vec = (1-alpha)*(abs(x-y).^2)/var + alpha*getHuberCost(reshape(x, [m n]), up, down, left, right, gamma);
    cost = sum(cost_vec);
    
    % Compute the gradient 
    deriv = 2*(1-alpha)*(x-y)/var + alpha*getHuberGradient(reshape(x, [m n]), up, down, left, right, gamma);
